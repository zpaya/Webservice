...
public class UserAPITest extends AutomationBase {
		...
		try {
			for (int i = excelRowNum; i < totalExcelRowNum; i++) {
				...
				if (execute.equalsIgnoreCase("Y")) {
					...
					testLog.log(LogStatus.INFO, "Rest WebService API: "+restURL);
					
					response = RestWebServiceUtil.GET(restURL);
					testLog.log(LogStatus.INFO, "API Response: "+response.asString());
	...
	public void tearDown(ITestResult result){
		if (result.getStatus()==ITestResult.FAILURE){
				...
				testLog.log(LogStatus.FAIL, "SampleTestFailed","Verification failed");
		}
